...
class Puzzle:
    ...
    def sanitize(self, input: Sequence[str]) -> list[str]:
        """Sanitizes the input. Called automatically if required to by the constructor"""
    ...
    def get_input(self, session: str, sanitized: bool | None = None) -> list[str]:
        ...
        if sanitized is None:
            sanitized = self.sanitized_input
        if os.path.exists(self.cachefile):
            ...
            with open(self.cachefile, "w") as f:
                f.write(c)
            c = c.split("\n")
        if self.sanitized_input:
            return self.sanitize(c)
        return c

    def get_solution(self, input: Sequence[str], timeout: int | None = None) -> list[str]:
        ...
        with futures.ThreadPoolExecutor(max_workers=2) as executor:
            futureA = executor.submit(self.solveA, input)
            resultA = None
            try:
                resultA = futureA.result(timeout)
            except (futures.TimeoutError, KeyboardInterrupt):
                print("Part A timed out")
                futureA.cancel()
            futureB = executor.submit(self.solveB, input)
            resultB = None
            try:
                resultB = futureB.result(timeout)
            except (futures.TimeoutError, KeyboardInterrupt):
    ...
    def expect(self, input: str | Sequence[str], expected: list[str | None]) -> bool:
    ...
    def submit(self, session: str, doit: bool = False, ans: list[str] | None = None) -> list[str]:
        """Grabs the input for the session and returns the answer to be submitted. Set doit to submit to the server"""
        if ans is None:
            ans = self.get_solution(self.get_input(session))
        if doit is False:
            return ans
        cookies = {"session": session}
        ra = None
        rb = None
        if len(ans) > 0 and ans[0] is not None:
            ra = requests.post(url=self.url + "/answer", cookies=cookies, data={"level": 1, "answer": ans[0]})
        if len(ans) > 1 and ans[1] is not None:
            rb = requests.post(url=self.url + "/answer", cookies=cookies, data={"level": 2, "answer": ans[1]})
        resA = ""
        if ra is None:
            resA = "No submission"
        elif ra.status_code != 200:
            resA = f"Got status {ra.status_code}"
        elif "not the right answer" in ra.text:
            resA = "Incorrect"
        elif "s the right answer" in ra.text:
            resA = "Correct"
        elif "already complete it" in ra.text:
            resA = "N/A"
        ...
        if rb is None:
            ...
            resB = f"Got status {rb.status_code}"
        elif "not the right answer" in rb.text:
            resB = "Incorrect"
        elif "s the right answer" in rb.text:
            resB = "Correct"
        elif "already complete it" in rb.text:
            resB = "N/A"
